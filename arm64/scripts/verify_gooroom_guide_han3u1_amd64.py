#!/usr/bin/env python3
"""Verify reconstructed gooroom-guide against the exact shipped AMD64 DEB."""
...
def main() -> int:
    ...
    if not control_identical:
        fatal.append({"kind": "control-fields", "target": target_control, "candidate": candidate_control})

    with tempfile.TemporaryDirectory(prefix="guide-amd64-equivalence-") as temp:
        temporary = Path(temp)
        ...
        for relative in sorted(set(target_manifest) | set(candidate_manifest)):
            ...
            if left["type"] != right["type"] or left["mode"] != right["mode"]:
                fatal.append({"kind": "type-or-mode", "path": relative, "target": left, "candidate": right})
                continue
            ...
            if not left.get("elf"):
                ...
                if not accepted:
                    fatal.append({"kind": "non-elf-payload", "path": relative, "target": left, "candidate": right})
                continue

            normalized_target_path = temporary / ("normalized-target-" + hashlib.sha256(relative.encode()).hexdigest())
            normalized_candidate_path = temporary / ("normalized-candidate-" + hashlib.sha256(relative.encode()).hexdigest())
            normalized_target = normalized_elf(target_path, normalized_target_path)
            normalized_candidate = normalized_elf(candidate_path, normalized_candidate_path)
            row = {
                "path": relative,
                "target_sha256": left["sha256"],
                "candidate_sha256": right["sha256"],
                "raw_identical": left["sha256"] == right["sha256"],
                "header_identical": elf_header(target_path) == elf_header(candidate_path),
                "dynamic_identity_identical": dynamic_identity(target_path) == dynamic_identity(candidate_path),
                "exported_symbols_identical": exported_symbols(target_path) == exported_symbols(candidate_path),
            ...
            elf_rows.append(row)
            if not row["verified"]:
                fatal.append({"kind": "normalized-elf-identity", "path": relative, "evidence": row})
    ...
    summary = {
        ...
        "non_elf_payload_identity": bool(non_elf_rows) and all(row["accepted"] for row in non_elf_rows),
        "elf_file_count": len(elf_rows),
        "normalized_elf_identity": bool(elf_rows) and all(row["verified"] for row in elf_rows),
        "raw_elf_identity": bool(elf_rows) and all(row["raw_identical"] for row in elf_rows),
