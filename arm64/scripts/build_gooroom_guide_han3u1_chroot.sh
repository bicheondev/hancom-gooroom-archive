...
while IFS= read -r package; do cp "$package" "$OUTPUT_DIR/debs/"; done < <(
  find "$ROOTFS/build" -maxdepth 1 -type f -name "*_${VERSION}_${ARCH}.deb" | LC_ALL=C sort
)
...
[ -n "$(find "$OUTPUT_DIR/debs" -maxdepth 1 -type f -name '*.deb' -print -quit)" ] || {
  echo "build produced no $ARCH DEBs for $VERSION" >&2; exit 2;
}
...
[ "$main_count" -eq 1 ] || { echo "expected exactly one $SOURCE package, got $main_count" >&2; exit 3; }
(
  cd "$OUTPUT_DIR/debs"
