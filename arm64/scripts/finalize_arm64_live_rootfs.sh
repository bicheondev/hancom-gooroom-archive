...
# The historical vendor repositories expose no binary-arm64 indexes. Keep the
# exact original lines as comments for provenance, but do not make every ARM64
# user hit a known 404 on apt update.
...
paths += sorted((root / 'etc/apt/sources.list.d').glob('*.list')) if (root / 'etc/apt/sources.list.d').exists() else []
for path in paths:
    if not path.exists():
        continue
...
# Runtime update source for the ARM64 port. The immutable build itself was made
# from the exact dated snapshot recorded in /usr/share/doc/hancom-gooroom-arm64.
...
The original Hancom/Gooroom binary repositories published AMD64 and
Architecture: all packages but no binary-arm64 index. Their original source
lines are retained as comments. Native ARM64 packages in this image were either
mapped to the exact Debian version or rebuilt from an exact source commit.
EOF
...
if command -v gtk-update-icon-cache >/dev/null; then
  ...
  done < <(find /usr/share/icons -mindepth 1 -maxdepth 1 -type d -exec test -f '{}/index.theme' ';' -print 2>/dev/null)
fi
...
rm -f "$ROOTFS/tmp/finalize-hancom-arm64.sh"
if [ "$rc" -ne 0 ]; then
  exit "$rc"
fi
