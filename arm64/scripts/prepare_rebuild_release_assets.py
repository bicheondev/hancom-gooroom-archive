...
def main() -> int:
    ...
    for (source, version), (result, result_path) in sorted(passed.items()):
        ...
        artifact_name = f"arm64-rebuild-{safe(source).replace('_', '-')}-{safe(version).replace('_', '-')}"
        expected = {
            row["filename"]: row
        ...
        with tempfile.TemporaryDirectory(prefix="arm64-rebuild-release-") as temporary:
            ...
            for filename, expected_row in sorted(expected.items()):
                matches = [path for path in artifact_root.rglob(filename) if path.is_file()]
                if len(matches) != 1:
                    errors.append(
                        {
                            "source": source,
                ...
                verification = (
                    load_json(verification_path) if verification_path.exists() else {}
                )
                binary_rows = [
                    row
                ...
                binary = binary_rows[0] if len(binary_rows) == 1 else {}
                packages.append(
                    {
            ...
            if source_failed:
                continue
            evidence_name = (
                f"{safe(source)}_{safe(version)}_arm64-build-evidence.tar.xz"
            )
