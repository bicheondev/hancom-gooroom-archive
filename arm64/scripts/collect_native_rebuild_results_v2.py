#!/usr/bin/env python3
"""Collect rebuild artifacts while preserving retry/dependency provenance.

The .deb payloads remain workflow/release assets. The branch stores compact
hashes, control verification, bounded diagnostics, and the exact dependency
repository hash used by each attempt. This makes dependency-triggered retries
idempotent instead of looping on the same repository state.
"""
...
MAX_DIAGNOSTIC_BYTES = 96 * 1024
MAX_DIAGNOSTIC_LINES = 360
DIAGNOSTIC_NAMES = (
    "workflow-build.log",
...
def bounded_tail(path: Path) -> dict[str, Any] | None:
    ...
    decoded = tail.decode("utf-8", "replace")
    all_lines = decoded.splitlines()
    lines = all_lines[-MAX_DIAGNOSTIC_LINES:]
    return {
        "filename": path.name,
        "sha256": hashlib.sha256(data).hexdigest(),
        "size": len(data),
        "truncated": len(data) > len(tail) or len(all_lines) > len(lines),
        "tail": "\n".join(lines),
    }
...
def main() -> int:
    ...
    for result_file in result_files:
            ...
            collection_errors.append(
                {
                    ...
                    "reason": "invalid-job-result-json",
        ...
        if not source or not version:
            collection_errors.append(
                {
                    "path": str(result_file),
                    "reason": "job-result-missing-source-identity",
                }
            )
            continue
        ...
        verification = (
            load_json(verification_path) if verification_path.exists() else None
        )
        ...
        dependency_packages_sha256 = first_nonempty(
            job.get("dependency_repository_packages_sha256"),
            (build_lock or {}).get("dependency_repository_packages_sha256"),
            (source_evidence or {}).get("dependency_repository_packages_sha256"),
        )
        dependency_release_lock_sha256 = first_nonempty(
            job.get("dependency_release_lock_sha256"),
            job.get("rebuild_release_lock_sha256"),
        )
        retry_reason = first_nonempty(
            job.get("retry_reason"),
            job.get("attempt_reason"),
            "initial-or-curated-build",
        )
        ...
        compact = {
            ...
            "dependency_repository_packages_sha256": dependency_packages_sha256,
    ...
    dependency_hashes = sorted(
        {
            row["dependency_repository_packages_sha256"]
            for row in results
            if row.get("dependency_repository_packages_sha256")
        }
    )
    summary = {
        "schema": 2,
        ...
        "passed_sources": [row["source"] for row in passed_rows],
        "failed_sources": [row["source"] for row in failed_rows],
        "dependency_repository_hashes": dependency_hashes,
        "complete_success": bool(results) and not failed_rows and not collection_errors,
    }
    ...
    return 0 if not collection_errors else 2
