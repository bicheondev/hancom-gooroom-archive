#!/usr/bin/env python3
"""Wire graphical boot gate v2 into final ISO and both QEMU probes."""
...
V1 = "arm64/scripts/configure_graphical_boot_gate.sh"
V2 = "arm64/scripts/configure_graphical_boot_gate_v2.sh"
...
def patch_workflow(path: Path) -> None:
    text = path.read_text(encoding="utf-8").replace(V1, V2)
    if V2 not in text:
        trigger = "      - 'arm64/scripts/finalize_arm64_live_rootfs_v2.sh'\n"
        if trigger not in text:
            raise RuntimeError("finalizer trigger marker is missing")
        text = text.replace(trigger, trigger + f"      - '{V2}'\n", 1)
...
"""
    ...
    evidence_section = text.find("Assemble immutable final evidence")
    if evidence_section < 0:
        raise RuntimeError("final evidence section is missing")
    if "work/live-finalization/graphical-boot-gate.json" not in text[evidence_section:]:
        copy_line = (
