...
# Generate and select Korean UTF-8 before packaging. The boot marker repeats
# the runtime check so a damaged locale archive cannot pass only at build time.
...
if [ ! -e "$ROOTFS/etc/systemd/system/display-manager.service" ]; then
  unit_path=""
  for candidate in \
...
session_count="$(find /usr/share/xsessions /usr/share/wayland-sessions \
  -maxdepth 1 -type f -name '*.desktop' 2>/dev/null | wc -l)"
[ "$session_count" -gt 0 ] || {
