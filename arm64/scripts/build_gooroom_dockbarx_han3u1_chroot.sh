...
[ "$actual_source" = "$SOURCE" ] || {
  echo "source identity mismatch: $actual_source" >&2
  exit 2
}
[ "$actual_version" = "$VERSION" ] || {
  echo "source version mismatch: $actual_version" >&2
  exit 2
}
...
while IFS= read -r metadata; do
  cp "$metadata" "$OUTPUT_DIR/metadata/"
done < <(
  find "$ROOTFS/build" -maxdepth 1 -type f \
    \( -name "*_${VERSION}_${ARCH}.buildinfo" \
       -o -name "*_${VERSION}_${ARCH}.changes" \) \
    | LC_ALL=C sort
)
...
for deb in "$OUTPUT_DIR"/debs/*.deb; do
  ...
  [ "$declared_source" = "$SOURCE" ] || {
    echo "wrong source in $deb: $declared_source" >&2
    exit 3
  }
  [ "$package" != "$SOURCE" ] || main_count=$((main_count + 1))
done
[ "$main_count" -eq 1 ] || {
  echo "expected exactly one $SOURCE package, got $main_count" >&2
  exit 3
}
