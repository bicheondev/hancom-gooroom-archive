...
done < <(jq -c '.packages[]' "$NIMF_AUTORITY")
...
grep -F 'theme_property_notified (NULL, NULL, NULL);' "$WORK/source/src/goorom-integration-applet.c"
grep -F 'notify::gtk-icon-theme-name' "$WORK/source/src/gooroom-integration-applet.c"
...
btest "$(dpkg-deb -f "$base/downloads/candidate.deb" Architecture)" = amd64
sudo rm -rf "$rootfs"
